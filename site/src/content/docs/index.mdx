---
title: Create Interactive Stories
description: A simple way to write interactive fiction
head:
  - tag: title
    content: Squiffy - Create interactive stories
template: splash
sidebar:
    hidden: true
prev: false
next:
    link: /getting-started/
    label: Getting Started
hero:
  title: Create interactive stories
  tagline: A simple, free tool for writing multiple-choice games and interactive fiction. No coding required.
  image:
    file: ../../assets/squiffy.png
  actions:
    - text: Get Started
      link: /getting-started/
      icon: right-arrow
    - text: Open the Editor
      link: https://app.squiffystory.com/
      icon: external
      variant: secondary
      attrs:
        target: _blank
---

import {Aside, Card, CardGrid} from '@astrojs/starlight/components';

## Why Squiffy?

<CardGrid>
  <Card title="Easy to Learn" icon="pencil">
    Write your story using simple text markup. Create links with `[[double brackets]]` for sections and `[single brackets]` for passages. No programming knowledge needed.
  </Card>
  <Card title="Publish Anywhere" icon="rocket">
    Squiffy generates pure HTML, CSS and JavaScript. Upload to your own website, itch.io, textadventures.co.uk, or anywhere that hosts static files.
  </Card>
  <Card title="Works Offline" icon="laptop">
    The editor runs entirely in your browser with no account required. Your work is automatically saved to your device.
  </Card>
  <Card title="Free and Open Source" icon="open-book">
    Squiffy is completely free to use. The source code is available on GitHub under the MIT license.
  </Card>
</CardGrid>

## Features

<CardGrid>
  <Card title="Sections and Passages" icon="document">
    Structure your story with **sections** that advance the narrative and **passages** that let players explore without moving forward.
  </Card>
  <Card title="Variables and State" icon="setting">
    Track player choices with attributes. Use conditionals to show different text based on what the player has done.
  </Card>
  <Card title="Animations" icon="star">
    Bring your story to life with fade, slide, and other animation effects when new text appears.
  </Card>
  <Card title="Dynamic Text" icon="random">
    Use random text, rotating sequences, and text replacement to add variety and replayability to your stories.
  </Card>
  <Card title="Player Input" icon="comment-alt">
    Capture text input from players with built-in validation. Ask for names, numbers, or any custom data.
  </Card>
  <Card title="JavaScript Integration" icon="seti:javascript">
    Extend your story with custom JavaScript for advanced game mechanics, calculations, or integrations.
  </Card>
  <Card title="Markdown Support" icon="seti:markdown">
    Format your text with Markdown. Add **bold**, *italics*, lists, links, and more. You can also use HTML for complete control.
  </Card>
  <Card title="Auto-Save" icon="approve-check-circle">
    Player progress is automatically saved to their browser. They can close the tab and return later to continue where they left off.
  </Card>
</CardGrid>

## Get Started

Ready to create your first interactive story? Head over to the [Getting Started](/getting-started/) guide to learn the basics, or jump straight into the [editor](https://app.squiffystory.com/) and start writing.

---

<Aside type="tip" title="Who made this?">
    My name is <a href="https://github.com/alexwarren">Alex Warren</a> (not the singer, I had the name first).
    I'm also the creator of <a href="https://textadventures.co.uk/quest">Quest</a>. I run
    <a href="https://textadventures.co.uk">textadventures.co.uk</a>, where you can play text adventure games
    and also submit your own, whichever system you created them with (even if it's not one I wrote!)
</Aside>