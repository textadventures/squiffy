...
|--------|-------------|---------|
| `{{attribute}}` | Display attribute value (single-word names) | `{{score}}` |
| `{{get "name"}}` | Get attribute value (any name, or when reading value set by helper in same section) | `{{get "player name"}}` |
