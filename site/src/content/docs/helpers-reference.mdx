...
| `{{append}}` | `"listVar" value` | Add element(s) to end of array | `{{append "inventory" "sword"}}` |
| `{{prepend}}` | `"listVar" value` | Add element(s) to start of array | `{{prepend "queue" "urgent"}}` |
| `{{pop}}` | `"listVar"` | Remove and return first element | `{{set "next" (pop "queue")}}` |
| `{{remove}}` | `"listVar" value` | Remove first matching value | `{{remove "inventory" "key"}}` |

### Querying arrays

| Helper | Parameters | Description | Example |
|--------|------------|-------------|---------|
| `{{contains}}` | `"listVar" value` or `(array ...) value` | Check if array contains value | `{{#if (contains "inventory" "key")}}...{{/if}}` |
| `{{length}}` | `"listVar"` or `(array ...)` | Get number of elements in array | `{{length "inventory"}}` |
