...
You can display the value of an attribute using `{{name}}` (if the attribute name is all one word), or `{{get "attribute name"}}` for multi-word attributes or when reading in the same section where it was set by a helper.
...
If you need to refer to an attribute value in the _same_ passage or section where it was modified by a helper like `{{inc}}`, `{{dec}}`, or `{{set}}`, you must use `{{get "attribute"}}` rather than just `{{attribute}}`.

This is because attribute values that can be accessed by name alone are captured once before rendering begins. Helpers that modify attributes during rendering update the internal state, but those changes won't be visible via the simple `{{attribute}}` syntax until the next section or passage.
...
Score: {{get "score"}}   // Use {{get}} here since we just modified it

+++Continue...

Score: {{score}}   // Now we can use the simple syntax
```
...
**Main Course:**
- [Order Chicken Curry](order_main, main=Chicken Curry, cost=12)
- [Order Vegetable Stir-Fry](order_main, main=Vegetable Stir-Fry, cost=10)
- [Order Beef Burger](order_main, main=Beef Burger, cost=14)
...
[drink]:
**Drinks:**
- [Soda](add_drink, drink=Soda, drink_cost=3)
- [Juice](add_drink, drink=Juice, drink_cost=4)
- [Water](add_drink, drink=Water, drink_cost=0)

[add_drink]:
{{inc &quot;total_cost&quot; drink_cost}}
Added {{drink}} {{#if (gt drink_cost 0)}}(${{drink_cost}}){{else}}(free!){{/if}}

Would you like [dessert]?

[dessert]:
**Desserts:**
- [Ice Cream](add_dessert, dessert=Ice Cream, dessert_cost=6)
- [Cake](add_dessert, dessert=Cake, dessert_cost=7)
- [Fruit](add_dessert, dessert=Fruit, dessert_cost=4)

[add_dessert]:
{{inc &quot;total_cost&quot; dessert_cost}}
Added {{dessert}} (${{dessert_cost}})
