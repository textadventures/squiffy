...
The main use for `{{get}}` is when you need to:
- Read attributes with multi-word names (containing spaces)
- Read an attribute whose name is determined by an expression (e.g., `{{get someVariable}}`)
